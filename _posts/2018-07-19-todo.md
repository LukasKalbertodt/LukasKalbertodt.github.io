...
Nikolas Matsakis introduced that pattern in [one of his blog posts about GATs](http://smallcultfollowing.com/babysteps/blog/2016/11/03/associated-type-constructors-part-2-family-traits/#introducing-type-families) (formerly known as ATC, associated type constructors). He used that pattern to show how powerful GATs really are. But as it turns out, we can use the same idea without GATs.
...
The only question is how to pass the input parameters to the "function". One possibility are GATs:
...
**The family trait pattern works by "delaying" the application of generic parameters. For each type constructor of interest, a non-generic family type is created that *represents* that type constructor. Through a trait, that family type can be used to construct types.**
...
fn generic_over_collection<C: for<T> CollectionFamily<T>>() {
    // Slightly more verbose to use the type constructor/"call" the
    // function.
...
Putting everything together, we get this solution (explanation in the comments):
...
// A family that represents a type constructor that always
// returns `T` (thus "id").
...
Additionally, specifying trait bounds for real `Item` type are still a pain due to the use of HRTBs:
