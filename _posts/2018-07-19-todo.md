...
With that, I declare this idea-space as exhausted. I'm pretty sure the solutions so far are the best we can get if we want to give the implementor complete freedom in choosing the `Item` type.
...
trait Iterator {
    type Item;
    fn next<'s>(&'s mut self) -> Option<Wrap<'s, Self::Item>>;
}
```
...
Yes. We do that by introducing marker types.
...
With these marker types, we can add a couple of neat impls and methods, like:
...
Of course, we would also add the corresponding impls for the two other markers:
- `impl<'a, T> From<&'a T> for Wrap<'a, T, RefMarker>`
- `impl<'a, T> From<&'a mut T> for Wrap<'a, T, RefMutMarker>`

Additionally, we could offer a more powerful way to extract the value:
...
Note that this only makes sense for traits that can be used via immutable reference (because we can't be sure to get more than that from the iterator). If you need to make sure that you get the `Item` by value, you can do that too:
...
- You can't return references to unsized types. The parameter `T` of `Wrap` needs to keep the `Sized` bound -- otherwise the `Owned(T)` variant wouldn't be legal. This has the consequence that we can't implement `WindowsMut` as we need to return `&mut [T]`. It might or might not be possible to work around this limitation with `unsafe` code -- I haven't tried.

- The type returned by `next()` is limited. For example, in `Lines`, we would like to return `io::Result<&str>`. But in that type, the lifetime parameter isn't on the outside, so we can't represent it with `Wrap` as shown above. We could of course add another variant for this case, but adding new variant results in quadratic code explosion. We need one marker trait for each variant which in turn needs to define an associated type for each variant. Extending `Wrap` thus get's unfeasable quickly.
...
This workaround is very different from the first two. It solves the problems associated with (A) and (B), but has a lot of own limitations. To me, it feels like this is the dirties of all three hacks.
...
In this post we tried to build a trait for streaming iterators which is general enough to allow owned items and items borrowed from `self` -- without using GATs. We discussed three possible workarounds:
...
(A) and (B) mainly suffer from the problem that HRTBs have to be used at some point. This becomes a problem together with the necessary bounds (`T: 'a`) to use a reference type `&'a T`: `for<'a> T: 'a` is equivalent to `T: 'static` which is a strict and in our case unnecessary requirement. (Interestingly, even with GATs, this is not trivially solvable.) Additionally, adding trait bounds to the `Item` type of the iterator is very verbose.
...
**Conclusion**: there exist a few workarounds for the lack of GATs. They work in several situations and might indeed be useful (especially for those of us who need GATS now), but they are overall very limited and have some disadvantages. Many problems are not properly solvable in today's Rust.
